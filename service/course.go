...
// DeleteStudent 删除学生
func DeleteStudent(cid uint, uid uint, user *entity.MUser) error {
	if !checkMCourseAuth(cid, user.ID, entity.Manager) {
...
func DeleteCourse(cid uint, user *entity.MUser) error {
	if !checkMCourseAuth(cid, user.ID, entity.Owner) {
		return errors.New("权限不足")
	}
